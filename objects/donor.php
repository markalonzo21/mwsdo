<?php
     class Donor{
        ...
        function insert($con){  
            ...
            $qry=$con->prepare($sql);   
            $qry->bind_param('issssssi',$this->id,$this->validdocument,$this->fullname,$this->email,$this->contact,$this->username,$this->password,$this->status);
            if($qry->execute()){
        ...
        function donor_info($id, $con){
            $sql="select d_id, validdocument, donorname, contact, status from tbl_donor where d_id=$id";
            $qry=$con->prepare($sql); 
            $qry->bind_result($this->id,$this->validdocument,$this->fullname,$this->contact,$this->status);
            $qry->execute();
            $data="";
            $r=0;
